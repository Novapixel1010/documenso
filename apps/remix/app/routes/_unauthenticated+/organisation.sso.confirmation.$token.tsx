...
export function ErrorBoundary({ error }: Route.ErrorBoundaryProps) {
  ...
  const errorMap = match(errorCode)
    .with('invalid-token', () => ({
      ...
      message: msg`The token is invalid or has expired. Please sign in to the organisation authentication portal to generate a new token.`,
    }))
...
export default function AcceptInvitationPage({ loaderData }: Route.ComponentProps) {
  ...
  const { mutate: linkOrganisationAccount, isPending } =
    trpc.enterprise.organisation.authenticationPortal.linkAccount.useMutation({
      onSuccess: async () => {
        await navigate(formatOrganisationLoginUrl(organisation.url));
    ...
    <div>
      <Card className="w-full max-w-2xl border">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="bg-muted/50 flex items-center justify-between gap-3 rounded-lg p-3">
              ...
              <Badge variant="secondary">{123456}</Badge>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-muted/50 flex items-center justify-between gap-3 rounded-lg p-3">
              <AvatarWithText
                ...
                secondaryText={`/o/${organisation.url}/signin`}
              />
          ...
          <div className="mb-4 flex items-center gap-x-2">
            ...
            >
              <Trans>I understand the consequences of accepting this request.</Trans>
            </label>
          </div>
        </CardContent>
        ...
        <CardFooter className="flex justify-end gap-3">
          ...
          <Button
            className="min-w-32"
            disabled={!isConfirmationChecked || isDeclining || isPending}
            loading={isPending}
